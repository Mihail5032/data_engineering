...
import ru.x5.process.PstDeduplicationFilter;
import ru.x5.process.PstTransactionKeySelector;
...
//        tableMap.put("E1BPTRANSACTEXTENSIO", TableIdentifier.of(SCHEMA, "bptransactextensio_pre"));
       // tableMap.put("E1BPLINEITEMVOID", TableIdentifier.of(SCHEMA, "raw_bplineitemvoid"));
        ...
        SingleOutputStreamOperator<RowData> rowData = deduplicated
                .keyBy(value -> String.valueOf(value.hashCode() % 10))
...
//        }
        ...
        // ВАЖНО: при раскомментировании — дедупликация на уровне транзакции
        // нужна для ВСЕХ PST-сегментов, не только PST_BPTRANSACTION.
        // Ключ дедупликации (rtl_txn_rk) одинаковый для всех сегментов одной транзакции.
...
//
//            // Дедупликация на уровне транзакции перед sink
//            Schema segmentSchema = pstSchemas.get(pstKey);
//            SingleOutputStreamOperator<RowData> dedupedStream = pstStream
//                    .keyBy(new PstTransactionKeySelector(segmentSchema))
//                    .process(new PstDeduplicationFilter())
//                    .setParallelism(10)
//                    .uid("pst-txn-dedup-" + pstKey.toLowerCase());
//
//            FlinkSink.forRowData(dedupedStream)
//                    .tableLoader(loader)
...
//        }
        ...
        // ТЕСТ: пишем только в PST_BPTRANSACTION (raw_bptransaction_test)
        // с дедупликацией на уровне отдельной транзакции (rtl_txn_rk)
        {
            String pstKey = "PST_BPTRANSACTION";
            OutputTag<RowData> tag = StreamSideOutputTag.getTag(pstKey);
            DataStream<RowData> pstStream = rowData.getSideOutput(tag);

            Schema pstSchema = pstSchemas.get(pstKey);

            // Дедупликация: keyBy(rtl_txn_rk) → PstDeduplicationFilter
            // Решает проблему: одна транзакция приходит в разных IDOC-ах,
            // DeduplicationFilter (уровень IDOC) её не ловит,
            // т.к. у каждого IDOC свой ключ (по первому E1BPTRANSACTION).
            SingleOutputStreamOperator<RowData> dedupedPstStream = pstStream
                    .keyBy(new PstTransactionKeySelector(pstSchema))
                    .process(new PstDeduplicationFilter())
                    .setParallelism(10)
                    .uid("pst-txn-dedup-" + pstKey.toLowerCase());
            ...
            FlinkSink.forRowData(dedupedPstStream)
                    .tableLoader(loader)
                    .writeParallelism(2)
                    .upsert(false)
                    .uidPrefix("sink-" + pstKey.toLowerCase())
        ...
        env.execute("XML Parser with correction 11 transaction test");
